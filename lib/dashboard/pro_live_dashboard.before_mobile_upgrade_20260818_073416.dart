...
class _ProLiveDashboardState extends State<ProLiveDashboard> {
  ...
  void initState() {
    ...
    signalSubscription = scannerController.signalStream.listen((signals) {
      ...
      setState(() {
        ...
        if (_signalFor(selectedPair) == null && rankedSignals.isNotEmpty) {
          selectedPair = rankedSignals.first.symbol;
        }
      });
    });
  ...
  Widget _watchlist() {
    final pinned = <ScanSignal>[];
    final automatic = <ScanSignal>[];
    final seen = <String>{};

    for (final signal in rankedSignals) {
      final symbol = _normalize(signal.symbol);

      if (watchlist.contains(symbol) && seen.add(symbol)) {
        pinned.add(signal);
      }
    }

    for (final signal in rankedSignals) {
      if (pinned.length + automatic.length >= 8) break;

      final symbol = _normalize(signal.symbol);

      if (seen.add(symbol)) {
        automatic.add(signal);
      }
    }

    final rows = [...pinned, ...automatic].take(8).toList();
